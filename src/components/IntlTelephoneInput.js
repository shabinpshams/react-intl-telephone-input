...
class IntlTelephoneInput extends Component {
  ...
  getExtension = number => {
    if (window.intlTelephoneInputUtils) {
      return window.intlTelephoneInputUtils.getExtension(
  ...
  getNumber = (number, format) => {
    if (window.intlTelephoneInputUtils) {
      return window.intlTelephoneInputUtils.formatNumber(
        this.getFullNumber(number),
        this.selectedCountryData.iso2,
        format,
      )
  ...
  formatNumber = number => {
    if (window.intlTelephoneInputUtils && this.selectedCountryData) {
      let format = window.intlTelephoneInputUtils.numberFormat.INTERNATIONAL
      ...
      if (
        ...
        format = window.intlTelephoneInputUtils.numberFormat.NATIONAL
      }

      number = window.intlTelephoneInputUtils.formatNumber(
        number,
        this.selectedCountryData.iso2,
        format,
      )
  ...
  updateValFromNumber = (number, doFormat, doNotify = false) => {
    if (doFormat && window.intlTelephoneInputUtils && this.selectedCountryData) {
      const format =
        !this.props.separateDialCode &&
        (this.nationalMode || number.charAt(0) !== '+')
          ? window.intlTelephoneInputUtils.numberFormat.NATIONAL
          : window.intlTelephoneInputUtils.numberFormat.INTERNATIONAL
  ...
  updatePlaceholder = (props = this.props) => {
    if (
      window.intlTelephoneInputUtils &&
      props.autoPlaceholder &&
      this.selectedCountryData
    ) {
      const numberType = window.intlTelephoneInputUtils.numberType[props.numberType]
      let placeholder = this.selectedCountryData.iso2
        ? window.intlTelephoneInputUtils.getExampleNumber(
            this.selectedCountryData.iso2,
            this.nationalMode,
            numberType,
          )
  ...
  isValidNumber = number => {
    ...
    if (window.intlTelephoneInputUtils) {
      return window.intlTelephoneInputUtils.isValidNumber(val, countryCode)
    }
  ...
  formatFullNumber = number => {
    return window.intlTelephoneInputUtils
      ? this.getNumber(
          number,
          window.intlTelephoneInputUtils.numberFormat.INTERNATIONAL,
        )
...
IntlTelephoneInput.defaultProps = {
...
export default IntlTelephoneInput
